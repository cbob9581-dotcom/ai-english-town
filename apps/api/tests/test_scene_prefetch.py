...
def test_cache_hit_and_miss() -> None:
    ...
    assert c.get("bakery", 0) is None
    c.put("bakery", 0, {"fills": []})
    assert c.get("bakery", 0) == {"fills": []}
...
def test_cache_lru_eviction() -> None:
    ...
    c.put("a", 0, {1}); c.put("b", 0, {2}); c.put("c", 0, {3})
    assert c.get("a", 0) is None and c.get("b", 0) == {2} and c.get("c", 0) == {3}


def _seed_memory(app) -> int:
    """把 plaza/bakery 预置为已访问（revision 稳定）：进场只改计数、不 bump revision，
    使 prefetch 键 (archetype, revision) 在测试期间保持命中。返回 seed 后的 revision。"""
    mem = app.state.memory
    events = app.state.events
    conn = events.connection
    now = datetime(2026, 8, 8, 12, 0, tzinfo=timezone.utc)
    for aid in ("plaza", "bakery"):
        events.append("seed", "scene.entered", {"archetypeId": aid, "sceneId": f"seed_{aid}",
                                                "generationId": f"g_{aid}", "revision": 1, "source": "seed"})
        with events.write_lock:
            mem.apply_memory_updates(conn, events, "local", scene_enter=aid, now=now)
            conn.commit()
    return mem.get_revision("local")


def _rev(app) -> int:
    return app.state.memory.get_revision("local")
...
async def test_spoke_entry_prefetches_back_and_next_enter_is_cached(tmp_path) -> None:
    ...
    assert app.state.prefetch.get("plaza", _rev(app)) is not None
...
async def test_hint_prefetches_and_later_enter_uses_cache_without_new_director_call(tmp_path) -> None:
    ...
    _seed_memory(app)  # plaza/bakery 均已知：进场不 bump revision → hint 预取在进场时命中缓存
    ws = FakeWS([
    ...
    with pytest.raises(asyncio.CancelledError):
        await task
    assert app.state.prefetch.get("bakery", _rev(app)) is not None
...
async def test_bad_cached_proposal_falls_back_to_director_and_invalidates(tmp_path) -> None:
    ...
    events, app = make_app(tmp_path, scenario="ok", scene_director=CountingDirector("ok"))
    _seed_memory(app)  # bakery 已知 → 进场不 bump revision → 坏提案在 fill 时被读到（命中）
    app.state.prefetch.put("bakery", _rev(app), {
    ...
    cached = app.state.prefetch.get("bakery", _rev(app))
